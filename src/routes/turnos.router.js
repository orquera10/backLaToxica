...
//me trae los turnos de los ultimos 2 meses
router.get("/mes", async (req, res) => {
  const startDate = new Date();
  startDate.setMonth(startDate.getMonth() - 2);
  const turnosMes = await Turnos.getReservasPorMes(startDate);
  res.json(turnosMes);
});
...
router.post("/", async (req, res) => {
  ...
  const diferencia_en_horas = nueva_hora_salida.diff(
    nueva_hora_entrada,
    "hours"
  );
  ...
  await Detalles.updateDetalle(nuevoDetalle._id, {
...
router.put("/:did/producto/:pid", async (req, res) => {
  ...
  const productoEncontrado = turno.detalle.productos_consumidos.findIndex(
  ...
  if (productoEncontrado !== -1) {
    ...
    total_consumido =
      turno.detalle.total_consumido + cantidad * precio_producto;
  } else {
    productos_nuevos = turno.detalle.productos_consumidos;
...
router.delete("/:did/producto/:pid", async (req, res) => {
  ...
  const indiceProducto = turno.detalle.productos_consumidos.findIndex(
    (item) => item.producto._id.toString() === productoId
  );
  ...
  if (indiceProducto !== -1) {
    const producto = await Productos.getProductoById(productoId);
    const totalProducto =
      turno.detalle.productos_consumidos[indiceProducto].cantidad *
    ...
    res
      .status(200)
      .json({
        message: "Producto eliminado del detalle del turno exitosamente.",
      });
  } else {
    ...
    res
      .status(404)
      .json({
        message: "El producto no está presente en el detalle del turno.",
      });
  }
});
